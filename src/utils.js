...
function extractModsField(node, fieldKey, fieldValue) {
    if (!node.children) return undefined;
    const modsIndex = node.children.findIndex(field => {
    ...
    const fieldName = node.children[modsIndex].value.children.find(modsField => {
...
function extractContent(nodeFields) {
    ...
    const contentFields = nodeFields.filter(element => {
